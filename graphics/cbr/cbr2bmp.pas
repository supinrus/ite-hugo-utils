	...
	if FileExists(palfn) then begin
		if (palfn[length(palfn)-3]='.') then begin
			if (((palfn[length(palfn)-2]='t') or (palfn[length(palfn)-2]='T'))
			 ...
			 and ((palfn[length(palfn)-1]='a') or (palfn[length(palfn)-1]='A'))
			 and ((palfn[length(palfn)]='w') or (palfn[length(palfn)]='W')) then begin
				assign(palf,palfn);
				reset(palf);
...
begin
	if (paramcount<>2) then begin
		writeln('Usage: cbr2bmp file.cbr palette.{raw,til,cgf,pal,lzp}');
		exit;
	end;
	fn:=paramstr(1);
	...
	for i:=0 to num-1 do begin
		...
		w:=buf[nofs]+buf[nofs+1]*256+buf[nofs+2]*256*256+buf[nofs+3]*256*256*256;
		h:=buf[nofs+4]+buf[nofs+5]*256+buf[nofs+6]*256*256+buf[nofs+7]*256*256*256;
		xpos:=buf[nofs+8]+buf[nofs+9]*256+buf[nofs+10]*256*256+buf[nofs+11]*256*256*256;
		ypos:=buf[nofs+12]+buf[nofs+13]*256+buf[nofs+14]*256*256+buf[nofs+15]*256*256*256;
		nofs:=nofs+16;
		mkheadbmp(w,h);
		for j:=0 to h-1 do
			for k:=0 to w-1 do
